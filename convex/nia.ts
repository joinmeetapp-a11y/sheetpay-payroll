...
// Nia now shares OpenAI with OCR (convex/ai.ts) and Cayla (convex/cayla.ts) —
// one API key, one billing surface, consistent latency and quality.
...
export const chat = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    const model = process.env.NIA_OPENAI_MODEL || DEFAULT_MODEL;
    let reply = "";
    try {
